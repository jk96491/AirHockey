...
public abstract class UITweener : MonoBehaviour
{
	...
	/// 아래 EventDelegate 와 다르게 매번 New 가 불려서 Allocation 이 되므로 계속 불리는 경우에는 EventDelegate 을 저장해서 아래것을 사용하는것을 추천
	/// </summary>

	public void RemoveOnFinished (EventDelegate.Callback del)
	{
		if (onFinished != null) onFinished.Remove( new EventDelegate(del));
		if (mTemp != null) mTemp.Remove(new EventDelegate(del));
	}
