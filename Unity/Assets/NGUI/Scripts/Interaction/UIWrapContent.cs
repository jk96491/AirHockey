...
public class UIWrapContent : MonoBehaviour
{
    ...
    //public delegate void OnDeInitializeItem(GameObject go, int wrapIndex, int realIndex);
    /// <summary>
    /// Width or height of the child items for positioning purposes.
    /// </summary>
    public int TopLeniency = 3;

    public int BotLeniency = 3;
    ...
    //public OnDeInitializeItem onDeInitializeItem;
    ...
    //protected virtual void DeUpdateItem(Transform item, int index)
    //{
    //    if (onDeInitializeItem != null)
    //    {
    //        int realIndex = (mScroll.movement == UIScrollView.Movement.Vertical) ?
    //            Mathf.RoundToInt(item.localPosition.y / itemSize) :
    //            Mathf.RoundToInt(item.localPosition.x / itemSize);
    //        onDeInitializeItem(item.gameObject, index, realIndex);
    //    }
    //}
}