...
public class UIDragDropItem : MonoBehaviour
{
    ...
    protected UIProgressBar mScrollBar;
    private float downAccel = .02f;
    private float upAccel = .02f;
    /// <summary>
    ...
    protected virtual void Update()
    {
        ...
        if (dragStart)
        {
            if (mTrans.position.y > .4f)
            {
                downAccel += 0.001f;
                if(mScrollBar)
                    mScrollBar.value -= downAccel;
            }
            else if (mTrans.position.y < -.9f)
            {
                upAccel += 0.001f;
                if(mScrollBar)
                    mScrollBar.value += upAccel;
            }
            else
            {
                downAccel = 0;
    ...
    protected virtual void OnDragDropStart()
    {
        ...
        var scroll = mParent.parent.GetComponent<UIScrollView>();
        if(scroll)
