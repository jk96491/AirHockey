...
public class UIScrollView : MonoBehaviour
{
    ...
    /// 
    /// </summary>
    public float stopMomentumValue = 0.1f;
    ...
    /// <summary>
    /// Strength of the spring effect.
    /// </summary>

    public float springPanelStrength = 80f;
    ...
    public float scrollSpeedReduce = 0.2f;
    ...
    protected virtual bool shouldMove
    {
        get
        {
            ...
            {
                if (b.min.y < clip.y - hy - .1f) return true;
                if (b.max.y > clip.y + hy + .1f) return true;
            }
            return false;
        }
    }
    ...
    public bool RestrictWithinBounds(bool instant, bool horizontal, bool vertical)
    {
        ...
        //Debug.Log("Bounds: " + b);
        //Debug.Log("ScrollView: " + constraint);
        ...
        if (constraint.sqrMagnitude > 0.1f)
        {
            if (!instant && dragEffect == DragEffect.MomentumAndSpring)
            {
                ...
                scrollSpeedReduce = 1f;
                SpringPanel.Begin(mPanel.gameObject, pos, 13f).strength = springPanelStrength;
            }
            else
            {
    ...
    public virtual void MoveTo(Vector3 relative)
    {
        mTrans.localPosition = relative;
        Vector2 co = mPanel.clipOffset;
        co.x = -relative.x;
    ...
    public void Drag()
    {
        ...
        if (enabled && NGUITools.GetActive(gameObject) && mShouldMove)
        {
            ...
            if (mPlane.Raycast(ray, out dist))
            {
                ...
                else mMomentum = Vector3.Lerp(mMomentum, mMomentum + offset * (0.01f * momentumAmount), 0.85f);
                ...
                else
                {
                    ...
                    if (constraint.magnitude - 1f > 0.0001f)
                    {
