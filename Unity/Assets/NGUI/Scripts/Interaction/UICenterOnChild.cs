...
public class UICenterOnChild : MonoBehaviour
{
    ...
    /// </summary>
    private GameObject prevCenterObject;
    ...
    void CenterOn(Transform target, Vector3 panelCenter)
    {
        ...
        if (ifCustom)
        {
            ...
            {
                if (mCenteredObject.name == "frontFaceBG")
                {
                    mCenteredObject = mCenteredObject.transform.parent.parent.gameObject;
                }
                else if (mCenteredObject.name == "FrontFace")
                {
                    mCenteredObject = mCenteredObject.transform.parent.gameObject;
                }

                if (child.gameObject == mCenteredObject)
                {
                    foreach (UIButton button in mCenteredObject.GetComponentsInChildren<UIButton>(true))
                    {
                        button.enabled = true;
                    }

                    if (prevCenterObject == null) prevCenterObject = mCenteredObject;
                    else if (mCenteredObject != null && prevCenterObject != mCenteredObject)
                    {
                        prevCenterObject = mCenteredObject;
                    }
                }
                else
                {
                    foreach (UIButton button in child.gameObject.GetComponentsInChildren<UIButton>(true))
                    {
