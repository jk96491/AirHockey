...
public class UIPanel : UIRect
{
	...
	void LateUpdate ()
	{
...
#endif
		{
            ...
            if(list == null)
            {
                Debug.LogError(string.Format("Panel UnKnown Error -> {0}   {1}",gameObject.name, 1199));
                return;
            }
			for (int i = 0, imax = list.Count; i < imax; ++i)
            {
                if (i >= list.Count)
                {
                    Debug.LogError(string.Format("Panel ArgumentOutOfRangeException Error -> {0}   {1}", gameObject.name, 1206));
                    continue;
                }
                ...
                if (null != list[i])
                    ...
                    Debug.LogError(string.Format("Panel UnKnown Error -> {0}   {1}", gameObject.name, 1213));
            }
			...
			for (int i = 0, imax = list.Count; i < imax; ++i)
			{
                ...
                if(i >= list.Count)
                {
                    Debug.LogError(string.Format("Panel ArgumentOutOfRangeException Error -> {0}   {1}", gameObject.name, 1225));
                    continue;
                }
                ...
                {
                    Debug.LogError(string.Format("Panel UnKnown Error -> {0}   {1}", gameObject.name, 1233));
                    continue;
                }
